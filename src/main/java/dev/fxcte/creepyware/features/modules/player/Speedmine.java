...
class Speedmine
    ...
    public Setting < Boolean > tweaks = this.register ( new Setting <> ("Speed", "Tweaks" , 0.0, 0.0, true, 0) );
    public Setting < Mode > mode = this.register ( new Setting < Object > ( "Mode" , Mode.PACKET , v -> this.tweaks.getValue ( ) ) );
    public Setting < Boolean > reset = this.register ( new Setting <> ("Speed", "Reset" , 0.0, 0.0, true, 0) );
    ...
    public Setting < Boolean > noBreakAnim = this.register ( new Setting <> ("Speed", "NoBreakAnim" , 0.0, 0.0, false, 0) );
    public Setting < Boolean > noDelay = this.register ( new Setting <> ("Speed", "NoDelay" , 0.0, 0.0, false, 0) );
    public Setting < Boolean > noSwing = this.register ( new Setting <> ("Speed", "NoSwing" , 0.0, 0.0, false, 0) );
    public Setting < Boolean > allow = this.register ( new Setting <> ("Speed", "AllowMultiTask" , 0.0, 0.0, false, 0) );
    public Setting < Boolean > doubleBreak = this.register ( new Setting <> ("Speed", "DoubleBreak" , 0.0, 0.0, false, 0) );
    public Setting < Boolean > webSwitch = this.register ( new Setting <> ("Speed", "WebSwitch" , 0.0, 0.0, false, 0) );
    public Setting < Boolean > silentSwitch = this.register ( new Setting <> ("Speed", "SilentSwitch" , 0.0, 0.0, false, 0) );
    public Setting < Boolean > render = this.register ( new Setting <> ("Speed", "Render" , 0.0, 0.0, false, 0) );
    ...
    void onTick ( ) {
        if ( this.currentPos != null ) {
            ...
            if ( Speedmine.mc.player != null && this.silentSwitch.getValue ( ) && this.timer.passedMs ( (int) ( 2000.0f * CreepyWare.serverManager.getTpsFactor ( ) ) ) && this.getPickSlot ( ) != - 1 ) {
                Speedmine.mc.player.connection.sendPacket ( new CPacketHeldItemChange ( this.getPickSlot ( ) ) );
            }
            if ( Speedmine.mc.player != null && this.silentSwitch.getValue ( ) && this.timer.passedMs ( (int) ( 2200.0f * CreepyWare.serverManager.getTpsFactor ( ) ) ) ) {
