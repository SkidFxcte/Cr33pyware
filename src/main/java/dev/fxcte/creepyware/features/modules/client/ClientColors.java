...
class ClientColors
        extends Module {
    public static ClientColors INSTANCE;
    public Setting< Boolean > rainbow = this.register ( new Setting <> ( "Rainbow" , Boolean.FALSE , "Rainbow ClientColors." ) );
    ...
    public
    void onTick ( ) {
        int ClientColorspeed = 101 - this.rainbowSpeed.getValue ( );
        float tempHue = this.hue = (float) ( System.currentTimeMillis ( ) % ( 360L * ClientColorspeed ) ) / ( 360.0f * (float) ClientColorspeed );
        for (int i = 0; i <= 510; ++ i) {
