...
class Nametags
    ...
    private final Setting < Mode > mode = this.register ( new Setting <> ( "Mode" , Mode.MINIMAL ) );
    ...
    void renderEnchantmentText ( ItemStack stack , int x ) {
        ...
        for (int index = 0; index < enchants.tagCount ( ); ++ index) {
            ...
            String encName = enc.isCurse ( ) ? TextFormatting.WHITE + enc.getTranslatedName ( level ).substring ( 11 ).substring ( 0 , 1 ).toLowerCase ( ) : enc.getTranslatedName ( level ).substring ( 0 , 1 ).toLowerCase ( );
