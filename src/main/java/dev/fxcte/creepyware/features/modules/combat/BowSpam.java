...
class BowSpam
    ...
    void onUpdate() {
        ...
        switch (this.mode.getValue()) {
            case AUTORELEASE: {
                if (! this.offhand && ! (BowSpam.mc.player.inventory.getCurrentItem().getItem() instanceof ItemBow) || ! this.timer.passedMs((int) ((float) this.delay.getValue() * (this.tpsSync.getValue() ? CreepyWare.serverManager.getTpsFactor() : 1.0f))))
                    break;
