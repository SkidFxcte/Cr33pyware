...
public class Webaura
    ...
    private final Setting<Boolean> server = this.register(new Setting<Boolean>("Speed", false));
    ...
    private final Setting<Boolean> upperBody = this.register(new Setting<Boolean>("Speed", false));
    private final Setting<Boolean> lowerbody = this.register(new Setting<Boolean>("Speed", true));
    private final Setting<Boolean> ylower = this.register(new Setting<Boolean>("Speed", false));
    private final Setting<Boolean> antiSelf = this.register(new Setting<Boolean>("Speed", false));
    ...
    private final Setting<Boolean> info = this.register(new Setting<Boolean>("Speed", false));
    private final Setting<Boolean> disable = this.register(new Setting<Boolean>("Speed", false));
    ...
    private boolean check() {
        ...
        return this.target == null || CreepyWare.moduleManager.isModuleEnabled("Freecam") && this.freecam.getValue() == false || !this.timer.passedMs(this.delay.getValue().intValue()) || this.switchMode.getValue() == InventoryUtil.Switch.NONE && Webaura.mc.player.inventory.currentItem != InventoryUtil.findHotbarBlock(BlockWeb.class);
    }
    ...
    private EntityPlayer getTarget(double range, boolean trapped) {
        ...
        for (EntityPlayer player : Webaura.mc.world.playerEntities) {
            if (EntityUtil.isntValid(player, range) || trapped && player.isInWeb || EntityUtil.getRoundedBlockPos(Webaura.mc.player).equals(EntityUtil.getRoundedBlockPos(player)) && this.antiSelf.getValue().booleanValue() || CreepyWare.speedManager.getPlayerSpeed(player) > this.speed.getValue())
                continue;
            if (target == null) {
                target = player;
