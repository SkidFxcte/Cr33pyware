    ...
    public void onUpdate() {
        if (Quiver.mc.player != null) {
            ...
            for (Integer slot : arrowSlots) {
                ...
                if (!Objects.requireNonNull(PotionUtils.getPotionFromItem(Quiver.mc.player.inventory.getStackInSlot(slot)).getRegistryName()).getPath().contains("strength")) continue;
                strengthSlot = slot;
            }
        }
    }
