...
class Killaura
    ...
    void doKillaura() {
        ...
        int wait = ! this.delay.getValue() || EntityUtil.holding32k(Killaura.mc.player) && ! this.teekaydelay.getValue() ? 0 : (wait = (int) ((float) DamageUtil.getCooldownByWeapon(Killaura.mc.player) * (this.tps.getValue() ? CreepyWare.serverManager.getTpsFactor() : 1.0f)));
        ...
        if (this.teleport.getValue()) {
            CreepyWare.positionManager.setPositionPacket(Killaura.target.posX , EntityUtil.canEntityFeetBeSeen(target) ? Killaura.target.posY : Killaura.target.posY + (double) target.getEyeHeight() , Killaura.target.posZ , true , true , ! this.lagBack.getValue());
        }
