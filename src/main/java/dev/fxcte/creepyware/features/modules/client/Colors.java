...
public class Colors
    ...
    public Setting<Integer> red = this.register(new Setting<Object>("Red", 255 , 0 , 255 , v -> this.rainbow.getValue() == false));
    public Setting<Integer> green = this.register(new Setting<Object>("Green", 255 , 0 , 255 , v -> this.rainbow.getValue() == false));
    public Setting<Integer> blue = this.register(new Setting<Object>("Blue", 255 , 0 , 255 , v -> this.rainbow.getValue() == false));
    public Setting<Integer> alpha = this.register(new Setting<Object>("Alpha", 255 , 0 , 255 , v -> this.rainbow.getValue() == false));
    public float hue;
    ...
    public void onTick() {
        ...
        if (ClickGui.getInstance ().colorSync.getValue ()) {
            CreepyWare.colorManager.setColor(INSTANCE.getCurrentColor().getRed(), INSTANCE.getCurrentColor().getGreen(), INSTANCE.getCurrentColor().getBlue(), ClickGui.getInstance().hoverAlpha.getValue());
        }
    }
