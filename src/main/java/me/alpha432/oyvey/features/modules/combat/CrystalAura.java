...
public class CrystalAura
    ...
    public CrystalAura() {
        super("CrystalAura", "yes", Module.Category.COMBAT, true, false, false);
    }
    ...
    private NonNullList<BlockPos> placePostions(float placeRange) {
        ...
        positions.addAll(CrystalAura.getSphere(new BlockPos(Math.floor(AutoCrystal.mc.player.posX), Math.floor(AutoCrystal.mc.player.posY), Math.floor(AutoCrystal.mc.player.posZ)), placeRange, (int) placeRange, false, true, 0).stream().filter(pos -> this.canPlaceCrystal(pos, true)).collect(Collectors.toList()));
        return positions;
    }
