...
public class AutoCrystal
    ...
    public final dev.fxcte.creepyware.util.Timer threadTimer = new dev.fxcte.creepyware.util.Timer();
    ...
    private final dev.fxcte.creepyware.util.Timer switchTimer = new dev.fxcte.creepyware.util.Timer();
    private final dev.fxcte.creepyware.util.Timer manualTimer = new dev.fxcte.creepyware.util.Timer();
    private final dev.fxcte.creepyware.util.Timer breakTimer = new dev.fxcte.creepyware.util.Timer();
    private final dev.fxcte.creepyware.util.Timer placeTimer = new dev.fxcte.creepyware.util.Timer();
    private final dev.fxcte.creepyware.util.Timer syncTimer = new dev.fxcte.creepyware.util.Timer();
    private final dev.fxcte.creepyware.util.Timer predictTimer = new dev.fxcte.creepyware.util.Timer();
    private final dev.fxcte.creepyware.util.Timer renderTimer = new dev.fxcte.creepyware.util.Timer();
    private final AtomicBoolean shouldInterrupt = new AtomicBoolean(false);
    private final dev.fxcte.creepyware.util.Timer syncroTimer = new dev.fxcte.creepyware.util.Timer();
    private final Map<EntityPlayer, dev.fxcte.creepyware.util.Timer> totemPops = new ConcurrentHashMap<EntityPlayer, dev.fxcte.creepyware.util.Timer>();
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (!this.justRender.getValue().booleanValue() && this.switchTimer.passedMs(this.switchCooldown.getValue().intValue()) && this.explode.getValue().booleanValue() && this.instant.getValue().booleanValue() && event.getPacket() instanceof SPacketSpawnObject && (this.syncedCrystalPos == null || !this.syncedFeetPlace.getValue().booleanValue() || this.damageSync.getValue() == DamageSync.NONE)) {
            ...
            if (packet2.getType() == 51 && AutoCrystal.mc.player.getDistanceSq(pos = new BlockPos(packet2.getX(), packet2.getY(), packet2.getZ())) + (double) this.predictOffset.getValue().floatValue() <= MathUtil.square(this.breakRange.getValue().floatValue()) && (this.instantTimer.getValue() == PredictTimer.NONE || this.instantTimer.getValue() == PredictTimer.BREAK && this.breakTimer.passedMs(this.breakDelay.getValue().intValue()) || this.instantTimer.getValue() == PredictTimer.PREDICT && this.predictTimer.passedMs(this.predictDelay.getValue().intValue()))) {
                ...
                if (this.predictFriendDmg.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.BREAK || this.antiFriendPop.getValue() == AntiFriendPop.ALL) && this.isRightThread()) {
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !CreepyWare.friendManager.isFriend(friend) || !((double) DamageUtil.calculateDamage(pos, friend) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        return;
                    }
                }
            ...
            if (packet5.getOpCode() == 35 && packet5.getEntity(AutoCrystal.mc.world) instanceof EntityPlayer) {
                this.totemPops.put((EntityPlayer) packet5.getEntity(AutoCrystal.mc.world), new dev.fxcte.creepyware.util.Timer().reset());
            }
    ...
    private boolean isRightThread() {
        return Util.mc.isCallingFromMinecraftThread() || !CreepyWare.eventManager.ticksOngoing() && !this.threadOngoing.get();
    }
        ...
        for (Entity entity : AutoCrystal.mc.world.loadedEntityList) {
            ...
            if ((double) selfDamage + 0.5 < (double) EntityUtil.getHealth(AutoCrystal.mc.player) && selfDamage <= this.maxSelfBreak.getValue().floatValue()) {
                ...
                for (EntityPlayer player : AutoCrystal.mc.world.playerEntities) {
                    ...
                    if (this.antiFriendPop.getValue() != AntiFriendPop.BREAK && this.antiFriendPop.getValue() != AntiFriendPop.ALL || !CreepyWare.friendManager.isFriend(player.getName()) || !((double) (damage = DamageUtil.calculateDamage(entity, player)) > (double) EntityUtil.getHealth(player) + 0.5))
                        continue;
                    ...
                    if (syncflag || this.currentDamage - (double) damageOffset > this.lastDamage || this.syncTimer.passedMs(this.damageSyncTime.getValue().intValue()) || this.damageSync.getValue() == DamageSync.NONE) {
                        ...
                        if (this.switchItem()) {
                            ...
                            if (this.foundDoublePop) {
                                this.totemPops.put(target, new dev.fxcte.creepyware.util.Timer().reset());
                            }
        ...
        for (BlockPos pos : BlockUtil.possiblePlacePositions(this.placeRange.getValue().floatValue(), this.antiSurround.getValue(), this.oneDot15.getValue())) {
            ...
            if (targettedPlayer != null) {
                ...
                if (this.calcEvenIfNoDamage.getValue().booleanValue() && (this.antiFriendPop.getValue() == AntiFriendPop.ALL || this.antiFriendPop.getValue() == AntiFriendPop.PLACE)) {
                    ...
                    for (EntityPlayer friend : AutoCrystal.mc.world.playerEntities) {
                        float friendDamage;
                        if (friend == null || AutoCrystal.mc.player.equals(friend) || friend.getDistanceSq(pos) > MathUtil.square(this.range.getValue().floatValue() + this.placeRange.getValue().floatValue()) || !CreepyWare.friendManager.isFriend(friend) || !((double) (friendDamage = DamageUtil.calculateDamage(pos, friend)) > (double) EntityUtil.getHealth(friend) + 0.5))
                            continue;
                        friendPop = true;
                        break;
                    }
